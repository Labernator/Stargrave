...
export const GearLabels = ({ gearList }: { gearList: Array<ModifiedGear | string> | undefined }) =>
    <React.Fragment>
        {gearList?.map((gearItem) => typeof (gearItem) === "string" ? getGearDetails(gearItem) : { ...getGearDetails(gearItem.name), gearSlots: gearItem.gearSlots }).sort((a, b) => sortAlgorithm(a) - sortAlgorithm(b)).map((gear) =>
